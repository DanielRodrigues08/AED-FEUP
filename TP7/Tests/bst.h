...
class BST {
    ...
    friend class BSTItrLevel<Comparable>;
    friend class iteratorBST<Comparable>;
    int findHeight(BinaryNode<Comparable>* node) const;
...
template <class Comparable>
int BST<Comparable>::findHeight(BinaryNode<Comparable>* node) const{
    if(node == NULL)
        return 0;
...
int BST<Comparable>::balance(const Comparable& x) const {
    BinaryNode<Comparable>* xf = find(x, root);
    if(xf == NULL)
        return 0;
    else
        return findHeight(xf->right) - findHeight(xf->left);
}
...
bool BST<Comparable>::isAVL() const {
    ...
    while (it!= this->end()){
        if(abs(balance(*it))>1) return false;
        it++;
    }
    return true;
}
